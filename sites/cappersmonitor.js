      ...
      return new Promise(function(resolve, reject) {
        scrapeIt(url, OPTIONS).then(({ data, response }) => {
            ...
            nba_items = _.chunk(nba_items, 3).map(item => {
              //return {game: item[0], handicapper: item[1], pick: item[2]}
              //return {game: item[0].substring(item[0].indexOf('pm')+5, item[0].length-1), handicapper: item[1], pick: item[2]}
            ...
            ncaab_items = _.chunk(ncaab_items, 3).map(item => {
              return {game: item[0], handicapper: item[1], position: item[2]}
            })
            if (platform == 'nba') {
              resolve(nba_items)
